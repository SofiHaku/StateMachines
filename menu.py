...
class Menu():
    ...
    def control(self, pos, clock, globals, screen):
        ...
        for event in pygame.event.get():
            ...
            # Обработка глайдера
            elif event.type == pygame.MOUSEBUTTONDOWN and self.control_glider(pos, self.gl.size, self.coord_x, self.coord_y):
                self.gla_t = True
                self.globals.gl = True

        clock.tick(60)
        return [self.gla_t]

    def control_figurs(self, pos, screen, mass):
        self.gl.draw_in_mass(pos[0], pos[1], screen, self.globals)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
            if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                self.globals.gl = False
    ...
    def draw_reg_speed(self, screen):
        ...
        pygame.draw.line(screen, (128, 128, 128), (825, self.coord_reg_y), (975, self.coord_reg_y), 5)
        pygame.draw.circle(screen,(128, 128, 128), (self.coord_reg_x, self.coord_reg_y), 8)
    ...
    def draw_all(self, screen):
        ...
        self.gl.draw_in_mass(self.coord_x, self.coord_y, screen, self.globals)
        pygame.draw.circle(screen, (128, 128, 128), (900, 75), 20)

    def control_glider(self, pos, size, x, y):
        """Проверка находится ли курсор в округе глайдера"""
